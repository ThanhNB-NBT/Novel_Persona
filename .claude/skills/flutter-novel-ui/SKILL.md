...
description: >-
  Build or edit the Flutter UI of THIS novel-reader app (Novel_Persona/app).
  Use this skill whenever you touch anything under app/lib/screens, app/lib/widgets/ or app/lib/theme.dart —
...
1. Đọc file đang sửa + `app/lib/theme.dart` (design tokens) + `app/lib/widgets/*.dart` (widget dùng chung; `widgets.dart` chỉ là barrel).
...
## Widget dùng chung (`widgets/common.dart`, `widgets/novel.dart`… — ưu tiên tái dùng)
...
Cần cái mới lặp lại ≥2 nơi → thêm vào module hợp lý trong `widgets/` (export sẵn qua barrel `widgets.dart`), đừng nhân bản inline.

## Kiến trúc (bám theo cái đang có)

- **State: Riverpod.** Data ở `app/lib/data/*.dart` (barrel `data.dart`). Đọc: `ref.watch(xProvider)` rồi `.when(loading/error/data:)`. Đa số là `FutureProvider.autoDispose`, có tham số thì `.family` (vd `chapterProvider(ChapterKey(...))`, `novelProvider(id)`). State ghi được (cài đặt) là `NotifierProvider` (vd `readerSettingsProvider`). Sau khi đổi dữ liệu server, `ref.invalidate(provider)` để refetch.
...
- **Supabase:** client toàn cục `sb` (`data/core.dart`). Chưa đăng nhập: `sb.auth.currentUser == null` → `context.push('/login')`. Truy vấn/RPC gói trong provider hoặc hàm ở `data/*.dart`, không rải vào widget.
- **Local prefs:** `prefs` toàn cục (SharedPreferences) qua helper trong `data/core.dart` (vd `saveChapterPercent`). Nhớ: `.order()` của supabase-dart **mặc định DESCENDING** — phải `ascending: true` nếu muốn tăng dần.
...
- Reader (`screens/reader/reader.dart`) tự resolve màu nền qua `s.resolve(platformBrightness)` — trong màn đọc dùng `col.bg/col.fg`, KHÔNG dùng ColorScheme của app.
