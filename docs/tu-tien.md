...
- **Linh căn** (migration 067 + 078): `elements[]` là BỘ HỆ ngũ hành CỐ ĐỊNH trời định
  (`cult_assign_root`, tạp phổ biến, dị/thiên căn 2%); Chuyển Linh Đan chỉ TRÁO bộ hệ.
  **BẬC là thang 7 nấc tách khỏi hệ** (078): Ngũ Hành Tạp(1)→Tứ→Tam→Song→Đơn(5)→
  Dị Linh Căn(6)→Tiên Linh Căn(7). Bậc GỐC theo số hệ (5 hệ→Tạp … 1 hệ→Đơn); dị căn
  bẩm sinh (kiem/loi/bang/phong/am) vào thẳng Dị, thien/hon vào Tiên (hon thêm hợp mọi
  công pháp). Đan luyện căn (`kind:linhcan`) cộng ĐIỂM vào cột `linh_can`; chi phí thăng
...
cache trạng thái đã phát xong. Canvas chỉ bổ sung lớp vân kiếp trôi, lóe sáng và nhánh lôi phụ
theo nhịp ba đạo để cảnh không đứng yên; tia đánh chính vẫn là WebP. `fx_lightning.json` chỉ chạy
nhanh sau khi thành công như sét tàn dư, không dùng làm thân kiếp lôi. Pha Tâm Ma có ma khí Canvas
xoắn quanh `heart_demon.webp`, đổi tím khi áp chế được và đỏ khi thất bại.
