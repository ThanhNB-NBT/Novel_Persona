...
## 11.7. Review 3 thay đổi của ChatGPT + chỉnh lại omission (2026-07-12)

ChatGPT thêm: (1) omission tự xưng vào fuse production, (2) scene analyzer chạy cả khi 2-pass tắt, (3) `--fresh` chạy full pipeline. Kết quả review/test:

- **(2) và (3) giữ nguyên** — code ổn, `--fresh 4` random chạy đẹp (4 chương chỉ 4 lỗi nhỏ).
- **(1) phải sửa 2 tầng:**
  - *Dính oan*: `source in zh` bắt cả 在下面 ("ở dưới"), 老夫老妻 ("vợ chồng già"); 晚辈→"hậu bối" hợp lệ bị chặn → chuyển sang regex có loại trừ + nhận thêm biến thể (hậu bối, chân thân/bản thể cho 本尊, bố mày/ông mày cho 老子). Test 4 ca oan.
  - *Kẹt job vĩnh viễn*: n1007 fail 3/3 lượt retry vì model lì 老子 → **rút omission khỏi fuse**, thay bằng `_fix_omissions`: sau dịch mỗi chunk, nếu thiếu dấu vết tự xưng thì MỘT lượt LLM nhận CÂU GỐC + bản dịch, sửa đúng câu tương ứng (Q3 đúng nghĩa — checker chỉ ra constraint, reviser nhận câu nguồn). Sửa không ăn thì giữ nguyên + lint báo cho người duyệt, không chặn pipeline.
- Eval `--fresh` thêm try/except từng chương — một chương lì không giết cả run (bug cũ: crash giữa chừng).
- Đo fuse mới trên 52 chương done còn content_zh: 10 omission THẬT + 2 bản phiên âm — **phát hiện thêm n971 (7,8%) là bản phiên âm thứ hai**, cần dịch lại như n1052.
...
## 13. Review sau tối ưu — việc còn hở (2026-07-12)

Kiểm chứng: `py_compile` đạt, evaluator self-check đạt, 16/16 regression test đạt.

### P0 — phải làm tiếp

1. **Omission self-reference chưa chặn production.** `老夫/老子/本座/在下...` mới được kiểm tra trong `eval_translation.py`; `_quality_fuse()` chưa dùng rule này nên output thiếu vẫn có thể lưu `done`. Chuyển constraint vào module production dùng chung và fail/revise khi thiếu.
2. **Scene contract bị tắt nhầm cùng pass tên riêng.** Sau ba chương ít tên mới, `twopass_active=False` làm worker ngừng phân tích speaker/addressee, dù hội thoại vẫn thay đổi. Tách scene analyzer khỏi name analyzer; chunk có hội thoại vẫn phải phân tích scene.
3. **`--fresh` chưa chạy pipeline production mới.** Evaluator hiện bỏ qua style bible, scene contract, previous context, soft fix và targeted revise. Cần dùng chung hàm dịch thuần với worker để A/B có giá trị.

### P1 — cần để nhất quán xuyên truyện

4. **Directed relationship chưa được lưu.** Scene chỉ sống trong một chunk; chưa có memory `speaker → addressee → self/address term` theo chapter range/confidence. Một bảng Postgres nhỏ là đủ, không cần graph/vector DB.
5. **Narrator term chưa có đường nhập.** Worker đã đọc `glossary_terms.narrator_term` nhưng app chưa có ô chỉnh và worker không tự ghi, nên cột này chưa tác động nếu DB rỗng. Thêm ô “Người kể gọi” vào màn Thuật ngữ.
6. **Reviewer xưng hô cần câu nguồn.** `_style_revise()` chỉ nhận câu Việt nên chỉ hợp sửa “chẳng/một cái/chăng”. Omission/sai speaker phải nhận source span + scene contract; không dùng reviewer văn phong để sửa lỗi nghĩa.

### P2 — gia cố nhỏ

7. `build_scene_line()` phải bỏ entry thiếu speaker **hoặc** addressee; hiện addressee `None` có thể lọt vào prompt.
8. Validate schema/độ dài style bible trước khi lưu; hiện bất kỳ JSON object nào cũng được giữ xuyên truyện.
9. Style bible có thể sinh từ chương bất kỳ được dịch đầu tiên; ưu tiên raw chương 1 hoặc lưu `source_chapter` để biết lúc nào cần tái tạo.
10. Analyzer nên dùng model/policy đã pin của truyện thay vì chain chung.

Thứ tự: production omission gate → scene không phụ thuộc two-pass → evaluator A/B thật → narrator UI → relationship memory → source-aware reviewer → P2.

### 13.1. Ba P0 đã triển khai (2026-07-12)

- Self-reference constraints đã chuyển vào production `worker.py`; `check_translation()`/`_quality_fuse()` chặn bản dịch làm mất `老夫`, `老子`, `本座`, `本尊`, `在下`, `晚辈`, `贫道`, `贫僧`, `哀家`, `朕`, `微臣`, `臣妾`. Evaluator tái dùng cùng một hàm, không còn hai bộ luật lệch nhau.
- Scene analysis đã tách khỏi trạng thái học tên: khi `twopass_active=False`, chunk có hội thoại vẫn chạy analyzer để lấy speaker/addressee/POV. Analyzer dùng model đã pin của truyện.
- `eval_translation.py --fresh` nay dry-run pipeline đầy đủ: model pinning, glossary, style bible, previous summary/tail, scene contract, quality fuse, soft fix và targeted revise; không ghi chapter/glossary/style vào DB.
- Regression mới: quality fuse phải chặn omission và scene analysis phải còn hoạt động khi two-pass tắt.

Kiểm chứng local: `py_compile` đạt, self-check đạt, 18/18 regression test đạt. Chưa gọi API chạy `--fresh`; lần A/B tiếp theo mới xác nhận chất lượng thực tế và chi phí call tăng do scene pass tiếp tục chạy ở chương có thoại.

### 13.2. Re-lint bộ 12 file baseline
