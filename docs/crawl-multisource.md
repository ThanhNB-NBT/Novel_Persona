...
> **Pha 2 — discovery ĐÃ code (2026-07-04, có trần):** adapter lấy danh sách id+tựa nhẹ; truyện MỚI mới gọi `fetch_novel_meta` lấy đủ metadata → upsert+dedup → enqueue dịch metadata (chỉ canonical); truyện đã có: bỏ qua. **Trần mặc định `settings.discover_new_per_cycle=10`** truyện đạt từ hàng đợi gộp/nguồn/chu kỳ (ranking toàn bảng có quota 10 riêng; có thể override từ cài đặt DB). Shuhaige lấy “Mới cập nhật” từ `/lastupdate/`; DDXS và Faloo lấy luân phiên từ các category được phép.
>
...
> **Discovery đa thể loại (2026-07-14):** mọi nguồn quét toàn bộ nhóm được app cho phép rồi mới qua bộ lọc chung; không khóa mỗi nguồn vào một loại. DDXS quét 玄幻/仙侠/武侠/网游/奇幻/科幻/悬疑; Faloo quét 玄幻奇幻/武侠仙侠/科幻网游/恐怖灵异 theo từng trang luân phiên để một category không chiếm hết pool.
>
> **Discovery truyện hoàn thành và bảng phụ (2026-07-14):** Shuhaige quét tổng lượt đọc `/allvisit/`, tổng đề cử `/allvote/`, TOP tổng hợp `/top.html`, mới cập nhật và riêng 7 thư viện `/quanben/{category}/`; mọi pool cùng qua dedup + bộ lọc thể loại chung. Faloo quét thư viện `/finish_0_0_0_{page}.html`. DDXS không công bố trang hoàn thành (các route phổ biến đều 404), nên adapter nhận diện từ tiêu đề 5 chương cuối (`完结`, `大结局`, `完本感言`...) khi đọc TOC; trạng thái này cũng miễn lọc truyện đang-ra dưới 200 chương.
>
> **Frontier discovery bền vững (migration 070):** mỗi nguồn/pool lưu `next_page` và hàng đợi ứng viên trong DB. Chu kỳ lẻ tiếp tục đào cursor sâu; chu kỳ chẵn luôn quét nóng lại trang 1 rồi quay về đúng cursor, không reset. Worker chỉ kiểm tra tối đa `2 × discover_new_per_cycle` ứng viên/nguồn/chu kỳ; Faloo dưới ngưỡng free được lưu `too_short` và hẹn kiểm tra lại sau 7 ngày thay vì tải lại mỗi vòng. Restart worker không mất vùng đã quét.
>
> **Mở rộng nguồn 2026-08-22 (migration 104):** probe thật bằng adapter của repo (curl_cffi impersonate chrome):
> - **Bật:** `ptwxz` (piaotia, 2694 chương probe OK) · `69shuba` (shuba, 1431 chương OK) — 2 nguồn đã seed từ 07-25, chờ kiểm chứng tới nay.
> - **Thêm `xslou`** (xinbiquge): book /{a}/{bid}/ hai tầng, mục lục phân trang index_{p}.html, content article.font_max, ranking /top/+/full/ — cần config `ranking_link_re` mới vì khuôn này để link trực tiếp /{a}/{bid}/ chứ không /book/....
> - **Thêm `qiushubang`** (biquge biến thể): book /index/{id}/, mục lục PHÂN TRANG /index/{id}/{p}/ (mỗi trang = khối "mới nhất" lặp + khối danh-sách-đầy-đủ CUỐI tăng dần → `toc_split` cắt lấy khối cuối), chương /read/{id}/{cid}.html div#content. Watermark tên-site xáo trộn ký tự lạ nhồi GIỮA câu (3 biến thể: Hán-xen-lạ, rác thuần, súp Latin) → xoá bằng config `junk_re` (4 regex, kiểm chứng sạch trên mẫu thật + không đụng văn thường). Không có route discovery dùng được (/top/, /allvisit/, /lastupdate/ đều 404) → nguồn chạy qua search/add tay.
> - **Chết/bị chặn lúc probe:** kujiang (JS-render), uuxs (DNS), xbiquge.la + biquge5200.cc (cert hết hạn), hetushu (Cloudflare), 81zw (server down). biqulao sống lại nhưng vẫn là mirror trùng shuhaige → bỏ như quyết định 07-25.
>
> **Template biquge mở rộng (2026-08-22):** config mới: `toc_page_path` (+`toc_split`, `toc_max_pages`) cho mục lục phân trang khuôn riêng; `junk_re` xoá watermark giữa câu; `ranking_link_re` cho xinbiquge ranking. Kèm theo: fetch_latest/fetch_ranking của nguồn KHÔNG-phải-shuhaige mà không khai đường dẫn riêng sẽ TẮT pool (trả []) thay vì 404 route mặc định mỗi chu kỳ (trước đây ném exception + log.exception mỗi vòng).
>
> **Tải chương SONG SONG trong một nguồn (2026-08-22):** `ensure_chapters_fetched` chạy `settings.crawl_fetch_workers` luồng (mặc định 3, chỉnh qua env; 1 = tuần tự như cũ). curl_cffi tạo handle curl theo từng luồng + db dùng client theo luồng nên an toàn; gặp SourceBlocked/SourceTransient hủy các chương chưa kịp chạy và dừng batch (giữ đúng ngữ nghĩa cũ). Mỗi luồng nghỉ 0.5s/chương → nhịp request tổng ≈ bản tuần tự × workers. Batch 20 chương: ~30-45s → ~12-18s/nguồn. Về RAM: crawler chỉ giữ transient vài MB HTML/trang; bộ nhớ nặng thật sự là model CT2 bên translator container, không phải crawler.
>
...
>   - jjwxc: user quyết không làm (chủ yếu truyện nữ).
>
> **Faloo free-only ĐÃ code (2026-07-13):** `FalooAdapter` dùng mobile HTML GB18030: metadata `/new_{id}.html`, catalog `/booklist/{id}.html`, content `.nodeContent`. Catalog đánh dấu VIP bằng `<span class="v_0">V</span>` → loại ngay khỏi mục lục; lúc fetch vẫn chặn lần hai nếu content chỉ là placeholder. `NovelMeta.chapter_count` và `novels.chapter_count_source` của Faloo luôn là **số chương free**, không phải tổng chương. Discovery/add tay chỉ nhận truyện có `free_count > faloo_free_chapter_threshold` (mặc định 500, chỉnh tại Quản trị → Crawl). Migration `066_source_faloo_free.sql`, test `worker/test/test_faloo.py`.
...
> | **faloo** ✅ | mobile HTML GB18030 sạch; catalog đầy đủ và đánh dấu VIP rõ bằng `v_0`; chương free ở `.nodeContent` | **ĐÃ THÊM free-only**; không crawl/bypass chương VIP |
> | quanben5, kujiang | kujiang = homepage 41 byte (JS-render/chặn, cần headless); quanben5 = timeout lặp (hạ tầng chập chờn) | KHÔNG cứu bằng HTTP thường |
...
>
> **Nguồn đang bật (8 dòng nguồn, 6 template):** `shuhaige` (biquge) · `ddxs` (dingdian) · `faloo` (free-only) · `ptwxz` (piaotia) · `69shuba` (shuba) · `xslou` (xinbiquge) · `qiushubang` (biquge + mục lục phân trang) · `fanqie` (adapter riêng decode font). Thêm truyện Faloo: `add --book-id <id số> --source faloo` (ID từ URL `/new_{id}.html`). qiushubang: add tay bằng ID từ URL `/index/{id}/`. fanqie: add tay bằng ID từ URL `/page/{id}`.
>
