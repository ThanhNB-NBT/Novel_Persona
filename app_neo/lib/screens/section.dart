...
/// "Xem tất cả" 1 mục — tải dần khi cuộn (logic giữ nguyên app cũ).
    ...
    return NeoScaffold(
      body: SafeArea(
        child: Column(children: [
          NeoAppBar(
            title: sectionTitles[widget.kind] ?? 'Truyện',
            actions: [
              IconButton(
                tooltip: 'Lọc truyện',
  ...
  Widget _body() {
    ...
    return ListView.separated(
      ...
      separatorBuilder: (_, i) =>
          i < _items.length - 1 ? const NeoDivider() : const SizedBox.shrink(),
      itemBuilder: (_, i) {
        if (i >= _items.length) {
        ...
        final n = _items[i];
        return NeoNovelRow(n: n, onTap: () => context.push('/novel/${n['id']}'));
      },
    );
  }
}
